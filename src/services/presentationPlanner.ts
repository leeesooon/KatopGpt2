...
function buildCodePlannerSystemPrompt() {
  return [
    ...
    '',
    '代码接口是固定的：',
    'module.exports.buildPresentation = async function buildPresentation({ pptxgen, input, assets, outputPath, workDir, tools }) { ... }',
    '',
    '可用内置工具：',
    '1. tools.addSemanticSvg(slide, kind, { x, y, w, h }, { title, items, opacity, theme }) 可绘制语义矢量图，kind 可用 learningGoals、conceptMap、processPath、caseCompare、practiceChecklist、ambientTexture。',
    '2. tools.addAmbientTexture(slide, { x, y, w, h }, { opacity, theme }) 可绘制低透明背景纹理。',
    '3. 工具会根据 input.themeId 派生配色，并使用 PPT 原生矢量形状绘制，不联网、不写文件；优先使用工具，不要手写大段 base64、SVG 字符串或 addImage 图片。',
    '',
    '硬性运行规则：',
    '1. 代码必须创建 const pptx = new pptxgen()，设置 pptx.layout = "LAYOUT_WIDE"，并 await pptx.writeFile({ fileName: outputPath })。',
    '2. 必须生成正好 input.slideCount 页，第一页是封面，最后一页是练习总结、行动清单或结尾页。',
    '3. 禁止依赖浏览器 DOM、window、document、canvas；可使用 Node 标准库，但不要联网、不要读写 outputPath/workDir 之外的路径。',
    '4. 使用 pptxgenjs 原生形状、文本、表格、手工绘制图表和 tools 语义矢量图；不要插入远程图片。',
    ...
    '',
    '培训课件质量规则：',
    '1. 输出培训课件风：学习目标、核心概念、步骤拆解、案例演示、练习检查和总结行动要清晰。',
    '2. 每页只承载 1 个主学习点；非封面/结尾页用“概念解释 + 示例/操作提示”组织正文，避免商务汇报式密集 bullet。',
    '3. 每页正文控制在 2-4 个短句或卡片；单条内容要能被学员快速扫读，长观点必须拆成概念、例子和提示三块。',
    '4. 没有真实数据时，不要编造百分比、金额、同比等具体数字；可使用结构性数字如“三步法、四个检查点”。',
    '5. 低页数规则：input.slideCount <= 5 时不要单独做目录页，优先安排封面、学习目标/核心概念、步骤/案例、练习/总结。',
    '6. 每页必须使用明确课件版式：双栏讲解、流程步骤、概念卡片、案例对照、练习清单或总结框，不要只放大标题。',
    '7. 排版必须稳定：统一页边距，固定标题区，同类卡片等高等宽，列间距一致，正文不要进入 y > 6.75 的底部区域。',
    '8. 代码中必须内置并复用 addTitle、addSectionLabel、addInfoCard、addStepFlow、addExampleBox、addChecklist 等 helper，减少每页随意摆坐标。',
    '9. 视觉风格要根据 input.themeId 调整配色和版式，不能所有主题都长一样。',
    '10. 正文卡片最多 2-3 行；长内容要拆短或降低字号，不要依赖 PowerPoint 自动撑高文本框。',
    '11. 非封面/结尾页主体内容必须覆盖约 65%-80% 的画布，不能出现一个大空容器只在顶部放几行字；页面必须有“主视觉区 + 内容区 + 辅助信息区”。',
    '12. 正文保持短句，但必须用编号、色块、图标、辅助说明、练习框、流程箭头或语义 SVG 补足视觉结构，不要只靠 bullet。',
    '13. 内容量要自适应版式：3 条以内做并列目标卡；4-5 条做 2x2 或 2x3 网格；概念+示例类页面做“概念解释 / 应用示例 / 练习提示”三栏。',
    '14. 培训类页面至少使用一种固定组件：流程步骤、案例框、练习清单、对比矩阵、关键提醒条或总结行动框。',
    '15. 每个非封面/结尾页至少调用一次 tools.addSemanticSvg 或 tools.addAmbientTexture；语义矢量图必须服务页面语义，例如目标页用 learningGoals，概念页用 conceptMap，步骤页用 processPath，案例页用 caseCompare，练习页用 practiceChecklist。',
    '',
...
function buildCodePlannerUserPrompt(options: GeneratePresentationCodeDeckOptions) {
  ...
  const basePrompt = [
    ...
    '',
    '请在代码中内置必要的布局函数：addTitle、addSectionLabel、addInfoCard、addStepFlow、addExampleBox、addChecklist、addFooter。',
    '可以直接使用 tools.addSemanticSvg 和 tools.addAmbientTexture；不要自己手写大段 SVG/base64，也不要用 addImage 插入 SVG。',
    '如果定义 addWrappedText/addInfoCard 等 helper，请先固定函数签名，所有调用必须严格匹配该签名；不要写 x:、y:、w:、h: 这种裸参数。',
    '建议结构：封面、学习目标、核心概念、步骤拆解、案例演示、练习/检查清单、总结行动；请按页数自动压缩或扩展。',
    '低页数结构：3-5 页时使用封面、学习目标/核心概念、步骤/案例、练习/总结，不要单独做目录页。',
    '每页都必须通过真实 PPT 元素表达内容：至少包含标题、一个主学习点、结构化视觉或练习/案例组件。',
    '优先输出可教学、可阅读、可复盘的页面；不要做空泛海报页，也不要堆商务汇报式 bullet。',
    '非封面/结尾页必须避免大面积空白：主体区域要被卡片、语义矢量图、流程线、案例框、练习清单或对比矩阵稳定填满。',
    '每页至少安排一个语义矢量图或轻纹理：学习目标页用 learningGoals，核心概念页用 conceptMap，步骤页用 processPath，案例页用 caseCompare，练习/总结页用 practiceChecklist 或 ambientTexture。',
    '如果某页只有 2-3 条短内容，请把它们做成并列卡片并搭配语义矢量图；如果有 4-5 条内容，请用 2x2/2x3 网格；如果是“概念 + 示例”，请做三栏。',
    '请在代码中定义常量 SLIDE_W = 13.333、SLIDE_H = 7.5、MARGIN = 0.65、TITLE_Y = 0.55、CONTENT_TOP = 1.55、FOOTER_Y = 6.95，并用这些常量计算网格、卡片和页脚位置。',
    '正文区域只能使用 CONTENT_TOP 到 6.65 之间的空间；页脚、页码和短标签才能出现在 FOOTER_Y 附近。',
  ]
  ...
  return [
    ...basePrompt,
    '',
    '上一次代码运行或质量审计没有达到要求，请基于错误/诊断修复，并输出修复后的完整代码，不要只给补丁。',
